import { Animated, Image, Modal, Platform, SafeAreaView, ScrollView, StatusBar, StyleSheet, Switch, Text, TouchableOpacity, View } from 'react-native'
...
const Index = ({ navigation }) => {
  ...
  if (Platform.OS === 'ios') {
    ...
    const [startOfWork1, setStartOfWork1] = useState(false)
    ...
    useFocusEffect(
      useCallback(() => {
        controlLocationInfo1()
        return () => {
        }
      }, [selectedDate.format('YYYY-MM-DD')])
    );
    const breakSheetRefIos = useRef();
    //BottomSheet
    ...
    async function controlLocationInfo1() {
    ...
    function handleMomentLocaleApple() {
      moment.locale(cultureStore?.culture);
      setStartDate(moment());
      setSelectedDate(moment());
    }
    useEffect(() => {
      handleMomentLocaleApple();
      requestPermissions1();
    ...
    const [backgroundModal1, setBackgroundModal1] = useState(false);
    const [foregroundModal1, setForegroundModal1] = useState(false);
    const requestPermissions1 = async () => {
      const foregroundPermissionGranted = await Location.getForegroundPermissionsAsync();
      if (foregroundPermissionGranted.canAskAgain === false && foregroundPermissionGranted.granted === false) {
        // setForegroundModal(true)
        const foregroundPermission = await Location.requestForegroundPermissionsAsync();
        if (foregroundPermission.status === 'granted') {
          const background = await Location.getBackgroundPermissionsAsync();
          if (background.canAskAgain === true && background.granted === false) {
            setBackgroundModal1(true)
          }
          else if (background.canAskAgain === false && background.granted === false) {
            setBackgroundModal1(true)
          }
          else if (background.canAskAgain === true && background.granted === true) {
          ...
          else if (background.canAskAgain === false && background.granted === true) {
            await Location.startLocationUpdatesAsync(LOCATION_TASK_NAME, {
              accuracy: Location.Accuracy.BestForNavigation,
              showsBackgroundLocationIndicator: true,
              foregroundService: {
          ...
          setForegroundModal1(true)
        }
      }
      else if (foregroundPermissionGranted.canAskAgain === true && foregroundPermissionGranted.granted === false) {
        const foregroundPermission = await Location.requestForegroundPermissionsAsync();
      ...
      else if (foregroundPermissionGranted.canAskAgain === true && foregroundPermissionGranted.granted === true) {
        const background = await Location.getBackgroundPermissionsAsync();
        if (background.canAskAgain === true && background.granted === false) {
          setBackgroundModal1(true)
        }
      ...
      else if (foregroundPermissionGranted.canAskAgain === false && foregroundPermissionGranted.granted === true) {
        const background = await Location.getBackgroundPermissionsAsync();
        if (background.canAskAgain === true && background.granted === false) {
    ...
    const requestBackgroundPermissions1 = async () => {
      const background = await Location.getBackgroundPermissionsAsync();
      if (background.canAskAgain === true && background.granted === false) {
        const backgroundPermission = await Location.requestBackgroundPermissionsAsync();
        if (backgroundPermission.status == 'granted') {
          setBackgroundModal1(false)
          await Location.startLocationUpdatesAsync(LOCATION_TASK_NAME, {
            accuracy: Location.Accuracy.BestForNavigation,
            showsBackgroundLocationIndicator: true,
            foregroundService: {
      ...
      ChangeAvailableStatus.Get(cultureStore?.culture).then(response => {
        if (response.status == 200) {
          if (response.data.responseCode == 200) {
            ...
            breakSheetRefIos?.current?.expand()
    ...
    function changeDriverStatus1(id) {
      let request = {
        statusType: id,
      }
      breakSheetRefIos?.current?.close()
      ChangeDriverStatus_Async.Get(request, cultureStore?.culture).then(response => {
        if (response.status == 200) {
          if (response.data.responseCode == 200) {
            getWorkOrders1()
            dispatch(setLoading(false));
            setStartOfWork1(true)
            setTimeout(() => {
              setStartOfWork1(false);
            }, 1000);
            // setBreakTimeData(response.data.data)
            // breakSheetRefAnd?.current?.expand()
          } else if (response.data.ResponseCode == 401) {
            // driverStore.setAvailable(!driverStore.available)
            responseStore.setRes401(true)
            responseStore.setResMessage(response.data.ResponseMessage)
    ...
    function cancelBreakStatus1(id) {
      breakSheetRefIos.current.close()
      let request = {
        statusType: id,
      }
    ...
    const culture_tr = {
      ...
      useBreakTime: 'Mesai İşlemleri',
      optionsText: 'Bu menüden mola ve mesai başlangıcı işlemlerinizi gerçekleştirebilirsiniz',
    ...
    useEffect(() => {
      ...
      const workOrderListChange = ref(db, 'workOrders/' + useAuthStore.getState().loginUser.id);
      if (isFocused) {
        getWorkOrders1();
    ...
    const [breakTimeSuccessMessage, setBreakTimeSuccessMessage] = useState(false)
    function breakTime1(item) {
      let request;
      ...
      ChangeAvailableStatus.Post(request, cultureStore?.culture).then(response => {
        if (response.status == 200) {
          if (response.data.responseCode == 200) {
            dispatch(setLoading(false));
            breakSheetRefIos?.current?.close();
            if (request?.breakType == 0) {
              driverStore.setAvailable(true)
              setTimer(false)
              setBreakTimeSuccess(true)
              setBreakTimeSuccessMessage(cultureResource.modalFinishMessage)
              getWorkOrders1()
            }
            else {
              driverStore.setAvailable(false)
              setBreakTimeSuccess(true)
              setBreakTimeSuccessMessage(cultureResource.modalSuccessMessage)
              setTimer(true)
              getWorkOrders1()
            }
          }
    ...
    function closeBottomSheet1() {
      breakSheetRefIos?.current?.close();
    }
    return (
      <>
        <View style={[styles.container, (driverStore.available ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
          {/* <SafeAreaView style={[(driverStore.available ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
            <View style={[styles.header, (driverStore.available ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
              <VText bold style={[{ fontSize: 18 }, (driverStore.available ? { color: Color.black } : { color: Color.white })]}>
                {driverStore.available ? cultureResource.headerTextAvailable : cultureResource.headerTextNotAvailable}
              </VText>
              <Switch
                trackColor={{ false: Color.white, true: Color.green }}
                thumbColor={driverStore.available ? Color.white : Color.dark}
                ios_backgroundColor={driverStore.available ? Color.green : Color.white}
                onValueChange={toggleSwitch1}
                value={driverStore.available}
              />
            </View>
          </SafeAreaView> */}
          <SafeAreaView style={[(workOrderData.currentStatus == -2 ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
            <View style={[styles.header, (workOrderData.currentStatus == -2 ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
              ...
              <TouchableOpacity style={{ padding: 10, borderRadius: 50, borderWidth: 1, backgroundColor: Color.base, borderColor: Color.darkGrey }} onPress={() => { toggleSwitch1() }}>
          ...
          <ScrollView style={styles.body} contentContainerStyle={{ paddingBottom: 100 }}>
            ...
            <View>
              {/* {workOrderData != null && tabStatus == -1 && workOrderData.workOrderList.filter(workorder => {
              return workorder.status === 2 || workorder.status === 3 || workorder.status === 4 || workorder.status === 6;;
            }).length > 0
              && (
              ...
              {/* {workOrderData != null && tabStatus == 1 && workOrderData.workOrderList.filter(workorder => {
              return workorder.status === 1 || workorder.status === 5 || workorder.status === -3 || workorder.status === -2 || workorder.status === -1;
            }).length > 0
              && (
                workOrderData.workOrderList.filter(workorder => {
                  return workorder.status === 1 || workorder.status === 5 || workorder.status === -3 || workorder.status === -2 || workorder.status === -1;
          ...
          <StatusBar backgroundColor={available ? Color.headerGrey : Color.darkGrey} barStyle={available ? 'dark-content' : 'light-content'} />

          {/* <Modal
            animationType="slide"
            transparent={true}
            visible={backgroundModal1}>
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                ...
                <VText regular style={{ marginTop: 20 }}>
                  {cultureResource.locationPermissionText}
                </VText>
                <VButton primary style={{ paddingHorizontal: 40, paddingVertical: 10, marginTop: 25 }}
                  onPress={() => { requestBackgroundPermissions1() }}>
                  <VText white bold>{cultureResource.locationPermissionButton}</VText>
                </VButton>
              </View>
            </View>
          </Modal> */}
          ...
          <Modal
            ...
            <View style={styles.centeredView}>
              <View style={styles.modalView}>
                ...
                <VButton primary style={{ paddingHorizontal: 40, paddingVertical: 10, marginTop: 25 }}
                  onPress={() => { requestPermissions1(), setForegroundModal1(false) }}>
        ...
        {/* {(timer && workOrderData?.available == false && workOrderData?.breakEnd != null && workOrderData?.breakEnd != undefined) && (
          <CountdownTimer targetDate={workOrderData?.breakEnd} setTimer={setTimer} breakTime={breakTime1} closeBottomSheet={closeBottomSheet1} cultureResource={cultureResource} />
        )} */}
        <Modal
          animationType="slide"
          transparent={true}
          visible={startOfWork1}
            ...
            <View style={{ flex: 1, }}>
              ...
              {(workOrderData.currentStatus == 0) && (
                <View style={{ flexDirection: 'row', alignItems: 'center', borderBottomWidth: 2, borderBottomColor: Color.greyBorder, paddingHorizontal: 16, paddingVertical: 8, }}>
                  ...
                  <TouchableOpacity onPress={() => { changeDriverStatus1(-1), breakSheetRefIos.current.close() }} style={{ flex: 0.3, alignItems: 'center', padding: 8, borderRadius: 30, backgroundColor: Color.green, flexDirection: 'row', justifyContent: 'center' }}>
              ...
              {(workOrderData.currentStatus != 0) && (
                <View style={{ flexDirection: 'row', alignItems: 'center', borderBottomWidth: 2, borderBottomColor: Color.greyBorder, paddingHorizontal: 16, paddingVertical: 8, }}>
                  ...
                  <TouchableOpacity onPress={() => { workOrderData?.breakEnd == null ? changeDriverStatus1(-2) : setShowToast1(true) }} style={{ flex: 0.3, alignItems: 'center', padding: 8, borderRadius: 30, backgroundColor: Color.red, flexDirection: 'row', justifyContent: 'center' }}>
                    <Image source={require('../../../assets/play-button.png')} style={{ width: 12, height: 12, marginRight: 3 }} />
              ...
              {breakTimeData != null && breakTimeData?.map((item, index) => {
                return (
                  <BreakTimeBottomPage item={item} key={index} cultureResource={cultureResource} changeDriverStatus={changeDriverStatus1} breakEnd={workOrderData.breakEnd} breakSheetRefAnd={breakSheetRefIos} currentStatusText={workOrderData?.currentStatusText} cancelBreakStatus={cancelBreakStatus1} />
                )
              })}
        ...
        <Portal>
          {breakTimeSuccess == true && (
            <ModalComponent alertClose={alertClose1} text={breakTimeSuccessMessage} cultureResource={cultureResource} />
          )}
        </Portal>
  ...
  if (Platform.OS === 'android') {
        ...
        foregroundSubscription = await Location.watchPositionAsync(
          ...
          (location) => {
            ...
            set(ref(db, "locations/" + useAuthStore.getState().loginUser.id), {
              d: useAuthStore.getState().loginUser.id + "|" + location.coords.latitude + "|" + location.coords.longitude,
            });
          }
        );
    ...
    function changeDriverStatus(id) {
      ...
      breakSheetRefAnd?.current?.close()
      ChangeDriverStatus_Async.Get(request, cultureStore?.culture).then(response => {
        if (response.status == 200) {
          if (response.data.responseCode == 200) {
            getWorkOrders()
    ...
    //   }
    //   ChangeAvailableStatus.Post(request, cultureStore.culture).then(response => {
    //     console.log('asd' , response.data , request)
    //     if (response.status == 200) {
    //       if (response.data.responseCode == 200) {
    //         dispatch(setLoading(false));
    //         breakSheetRefAnd?.current?.close();
    ...
    return (
      <>
            ...
            <SafeAreaView style={[(workOrderData.currentStatus == -2 ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
              <View style={[styles.header, (workOrderData.currentStatus == -2 ? { backgroundColor: Color.headerGrey } : { backgroundColor: Color.dark })]}>
                ...
                {/* <Switch
                  ...
                  value={driverStore.available}
                /> */}
                <TouchableOpacity style={{ padding: 10, borderRadius: 50, borderWidth: 1, backgroundColor: Color.base, borderColor: Color.darkGrey }} onPress={() => { toggleSwitch() }}>
        ...
        {/* {(timer && workOrderData?.available == false && workOrderData?.breakEnd != null && workOrderData?.breakEnd != undefined) && (
          <CountdownTimer targetDate={workOrderData?.breakEnd} setTimer={setTimer} breakTime={breakTime} closeBottomSheet={closeBottomSheet} cultureResource={cultureResource} />
        )} */}
      </>
    )
  }
