      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <View style={{ flex: 1, backgroundColor: Color.white, paddingTop: Constants.statusBarHeight }}>
          ...
          <View style={{ margin: 20 }}>
            <VText bold style={{ fontSize: 15, marginLeft: 10 }}>{cultureResource.name}</VText>
            <View style={{ marginTop: 10, marginBottom: 10 }}>
              <VInput placeholder={name} maxLength={11} keyboardType='default' value={nameText} onChangeText={(text) => { setNameText(text) }} />
            </View>
            <VText bold style={{ fontSize: 15, marginLeft: 10 }}>{cultureResource.surname}</VText>
            <View style={{ marginTop: 10 }}>
              <VInput placeholder={lastName} maxLength={11} keyboardType='default' value={lastNameText} onChangeText={(text) => { setLastNameText(text) }} />
            </View>
          </View>
          <TouchableOpacity onPress={() => { nameText != '' && nameText != null && lastNameText != '' && lastNameText != null ? hide() : {} }} style={{ paddingVertical: 20, justifyContent: 'center', alignItems: 'center', borderRadius: 30, backgroundColor: Color.purple, position: 'absolute', bottom: 100, right: 20, left: 20 }}>
