...
const CarDocumentsComponents = ({ item, keyItem, navigation ,openBottomSheet , cultureResource }) => {
    ...
    return (
        <>
            <TouchableOpacity key={keyItem} onPress={() => { Linking.openURL(item.url) }} style={{ padding: 20, flexDirection: 'row', alignItems: 'center', borderBottomColor: Color.greyBorder, borderBottomWidth: 1 }}>
                ...
                <View style={{ marginLeft: 10 }}>
                    ...
                    {item.isTerm && (
                        <VText greyText semibold style={{ fontSize: 14, }}>{cultureResource.expirationDate}{moment(item.expireDate).format('LL')} </VText>
                    )}
                    {item.isTerm == false && (
                        <VText greyText semibold style={{ fontSize: 14, }}>{cultureResource.expirationDate} {cultureResource.unlimited}</VText>
                    )}
                </View>
            </TouchableOpacity>
...
export default CarDocumentsComponents
